...
const CurrencyInput: React.FC<Props> = ({
  ...
  const getMaxInputValue = async () => {
    if(!useMax) {
      return '0.00';
    }

    const { balanceMatic, balanceUSD, getFeeEstimate } = useLink();
    const feesInGwei = await getFeeEstimate();
    const feesInWei = ethers.utils.formatUnits(feesInGwei, "gwei");
    const fees = Number(ethers.utils.formatEther(feesInWei));
    if (balanceMatic < fees) {
      return "0.00";
    }
    if(isInputInCryptoCurrency()){
      return (balanceMatic - fees).toFixed(4);
    } 

    return (balanceUSD - fees / cryptoPrice).toFixed(2);
  }
